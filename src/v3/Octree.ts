...
function GetCoordsOffset(x: number, y: number, z: number, depth: number): number {
  return (
    ((x >> depth) & 0x1)
    | (((y >> depth) & 0x1) << 1)
    | (((z >> depth) & 0x1) << 2)
        ...
        for (let j = 0; j < 2; j++) {
          ...
          if(ratio >= 0) {
            ...
            if ((0 <= coords[b]) && (coords[b] < side) && (0 <= coords[c]) && (coords[c] < side)) {
              // console.log(`hit at ${a}, ${b}, ${c} (${side}) => ${coords.join(', ')}`);
...
export class Octree {
  static computeMaxOctreeSize(depth: number): number {
    return (depth === 0) ? 33 : this.computeMaxOctreeSize(depth - 1) * 8 + 33;
  }
  ...
  static computeSideDepth(side: number): number {
    const depth: number = Math.round(Math.log2(side)) - 1;
  ...
  get side(): number {
    return 0x1 << (this.depth + 1); // Math.pow(2, this.depth)
  }
  ...
  setValueAt(x: number, y: number, z: number, value: Uint8Array): void {
    ...
    while(depth >= 0) {
      const coordsOffset: number = GetCoordsOffset(x, y, z, depth);
  ...
  getValueAt(x: number, y: number, z: number): Uint8Array {
    ...
    while(depth >= 0) {
      coordsOffset = GetCoordsOffset(x, y, z, depth);
  ...
  computeRay(ray: Ray): Uint8Array {
    ...
    let hits: Float32Array[] | null = ray.hitCube(new Float32Array([0, 0, 0]), this.side);
    if(hits !== null) {
      const hit1: Float32Array = hits[0];
      const hit2: Float32Array = hits[1];

      const x: number = Math.floor(hit1[0]);
      const y: number = Math.floor(hit1[1]);
      const z: number = Math.floor(hit1[2]);
      ...
      while(depth >= 0) {
        ...
        if((this.data[dataIndex] >> coordsOffset) & 0x1) {
          ...
          // TODO continue here => add hitCubeIn and hitCubeOut to detect entry and out point of ray
          console.log('found color');
          console.log(this.data.subarray(_dataIndex, _dataIndex + 4).join(', '));
          console.log('depth', depth);
          let side: number = 0x1 << (depth + 1);
          console.log('side', side);
          hits = ray.hitCube(hit1, side);
          console.log(hits);
          break;
          // return this.data.subarray(_dataIndex, _dataIndex + 4);
        }
  ...
  /**
   * Problems: hard to order colors + explore 'all' hit subtrees
   * this.recursiveComputeRay(ray, 0, this.depth, 0, 0, 0);
   */
  protected recursiveComputeRay(ray: Ray, dataIndex: number, depth: number, x: number, y: number, z: number): void {
    let _dataIndex: number = dataIndex + 1;
    const position: Float32Array = new Float32Array([x, y, z]);
    let side: number = 0x1 << (depth + 1);
    ...
    for(let _z = 0; _z < 2; _z++) {
      position[2] = (position[2] & mask) | (_z << depth);
      for (let _y = 0; _y < 2; _y++) {
        position[1] = (position[1] & mask) | (_y << depth);
        for (let _x = 0; _x < 2; _x++) {
          position[0] = (position[0] & mask) | (_x << depth);
          // console.log(position);
          const hits: Float32Array[] | null = ray.hitCube(position, side);
          if(hits !== null) {
            // console.log(hits);
            if((this.data[dataIndex] >> i) & 0x1) {
              this.recursiveComputeRay(
                ray,
                ReadAsUint32(this.data, _dataIndex),
                depth - 1,
              ...
              console.log('hit color', this.data.subarray(_dataIndex, _dataIndex + 4));
            }
          }
...
function generateRainbowSphereOctree(side: number): Octree {
  const tree: Octree = Octree.fromSide(side);
  const halfSide: number = side / 2;
  for(let z = 0; z < side; z++) {
    for (let y = 0; y < side; y++) {
      for (let x = 0; x < side; x++) {
        ...
        if(radius <= halfSide) {
          // console.log('set at', x, y, z);
          tree.setValueAt(x, y, z, new Uint8Array([x / side * 256, y / side * 256, z / side * 256, 255]));
        }
      }
    }
  }
...
function testRays() {
  ...
  let tree = generateRainbowSphereOctree(128).compact();
